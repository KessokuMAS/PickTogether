...
const GoogleCallbackPage = () => {
  ...
  useEffect(() => {
    ...
    const socialLoginFlow = async () => {
      ...
      try {
        console.log("구글 로그인 시작 - 인가 코드:", authCode);

        const googleAccessToken = await getAccessToken(authCode);
        if (!googleAccessToken) {
          throw new Error("Google accessToken을 못 받았습니다.");
        }

        const data = await getMemberWithAccessToken(googleAccessToken);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-md">
        <div className="flex items-center justify-center">
          ...
          <span className="text-lg">구글 로그인 처리 중...</span>
        </div>
        {authCode && (
